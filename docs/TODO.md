...
- **Netlify Basic-Auth placeholder credentials removed** — `frontend/public/_headers` no longer carries a literal committed Basic-Auth rule. Real credentials are now generated at build time by `frontend/scripts/generate-headers.mjs` (postbuild, from `NETLIFY_BASIC_AUTH_CREDENTIALS` env var) into `dist/_headers` only (gitignored). See SECURITY_AND_KEYS.md.
